...
Tags: NBA, sports, stats, widgets, plugins, plugin, sidebar, widget, teams, NBA stats, hockey, hockey stats, hockey team stats, NBA standings, hockey standings
...
NBA Team Stats plugin can be added to any blog with a dynamic sidebar. Once installed, the blogger can choose their favorite NBA Team 
and the type of display they want (compact or large). The stats are updated each week of the NBA regular season, so that blog 
visitors can keep track of all the important NBA team standings and stats. Additionally, teams qualifying for post-season play are denoted automatically 
in each stats table with the the standard NBA symbols.
...
Generally speaking, we have no outward preference where our plugin appears, as long as the content of the particular blog is legal in its respective jurisdiction. For example, 
many bloggers like to post sports stats on <a href="http://www.sportsbetting3.com">sports betting</a> websites, which is permissable where legal. Similarly, use of the plugin 
on an <a href="http://www.plazacasinos.com">online casinos</a> website may not be necessarily relevant, but is tolerable if used in an appropriate context. 
...
1. Sample Screen Shot of Stats, with and without optional mini news scroller. `/tags/1.1.2/screenshot-1.png`<br />

2. Another Example of the Stats Plugin:`/tags/1.1.2/screenshot-2.png`<br />


== Changelog ==

= 1.1 =
ALL NEW News Feed, with top headlines for NBA fans that update regularly throughout the day. The scroller is very simple: it displays 4 news items which are clickable and open a new browser page to see the actual story. You can 
activate the scroller in your Blog settings control panel for NBA-Team-Stats. Blog users should have Adobe Flash enabled browsers to see the news feed. Also, the use of a Flash feed 
...
= 0.9.2 = 
Bug in Stats Display File Corrected.

= 0.9.1 = 
Fixed images, updated code and Readme.txt.

= 0.8.1 =
Fixed some images that were not appearing.
